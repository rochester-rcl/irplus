...
<body class="yui-skin-sam">
    ...
    <div id="doc2">
        ...
        <h3>Upload file(s) to Group Workspace: ${groupWorkspace.name}<c:if test="${folderId > 0}">${groupWorkspaceFolder.fullPath}</c:if></h3>
        ...
        <div id="bd">  
            ...
            <form method="get" action="${myFoldersUrl}" name="cancelAddFilesForm">
	            <input type="hidden" value="${folderId}" name="groupWorkspaceFolderId"/>
	            <input type="hidden" value="${groupWorkspace.id}" name="groupWorkspaceId"/>
	            <input type="hidden" value="GROUP_WORKSPACE" name="tabName"/>
	        </form>
            
            <c:url var="fileUpload" value="/user/uploadGroupWorkspaceFiles.action"/>

...
/**
 * Test getting the text from an excel document
 * 
 * @author Nathan Sarr
 * 
 */
@Test(groups = { "baseTests" }, enabled = true)
public class DefaultExcelTextExtractorTest {
	 ...
	 * @throws Exception 
	 */
	public void testIndexExcelDocument() throws Exception {
		...
		String excelFile = properties.getProperty("excel_file");
		
		File f1 = new File(baseLocation + excelFile);
		assert f1 != null : "File should not be null";
		assert f1.canRead(): "Should be able to read the file " 
			+ f1.getAbsolutePath();

		FileInfo info = repo.getFileDatabase().addFile(f1, "indexed_excel_file");
		info.setExtension("xls");

		FileTextExtractor documentCreator = new DefaultExcelTextExtractor();
		...
		try {
			writer = new IndexWriter(dir, new StandardWithACIIFoldingFilter(), true,  

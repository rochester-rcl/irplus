...
public class InstitutionalCollectionUtils {
	 ...
	 */
	public static boolean canMoveToCollection(Collection<FileSystem> objectsToMove, FileSystem destination)
	{
		
		if( !destination.getFileSystemType().equals(FileSystemType.INSTITUTIONAL_COLLECTION))
	    {
		    return false;	
		}
		...
		{
			if( fileSystemObject.getFileSystemType().equals(FileSystemType.INSTITUTIONAL_COLLECTION))
			{
	 ...
	 */
	public static boolean isInstitutionalPublicationToBeMoved(Collection<FileSystem> objectsToMove, FileSystem destination)
	{
		
		if( !destination.getFileSystemType().equals(FileSystemType.INSTITUTIONAL_ITEM))
	    {
		    return false;	
		}
		...
		for(FileSystem fileSystemObject : objectsToMove)
		{
			if( fileSystemObject.getFileSystemType().equals(FileSystemType.INSTITUTIONAL_ITEM))
			{
			    if( fileSystemObject.getId().equals(destination.getId()))
			    {
			    	isItemToMove = true;
